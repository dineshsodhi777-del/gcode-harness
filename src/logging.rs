//! Logging infrastructure for gcode
//!
//! Logs to ~/.gcode/logs/ with automatic rotation
//!
...
impl Logger {
    fn new() -> Option<Self> {
        ...
        let path = log_dir.join(format!("gcode-{}.log", date));
    ...
    fn write(&mut self, level: &str, message: &str) {
        ...
        if let Err(err) = self.file.write_all(line.as_bytes()) {
            eprintln!("gcode logger write failed: {err}");
            return;
        }
        if let Err(err) = self.file.flush() {
            eprintln!("gcode logger flush failed: {err}");
        }
    }
}
...
/// Log a debug message (only if GCODE_TRACE is set)
#[expect(
    clippy::collapsible_if,
    reason = "Debug logging keeps env gating and logger access explicit"
)]
pub fn debug(message: &str) {
    if std::env::var("GCODE_TRACE").is_ok() {
        if let Ok(mut guard) = LOGGER.lock() {
            if let Some(logger) = guard.as_mut() {
        ...
        && let Ok(entries) = fs::read_dir(&log_dir)
    {
        ...
        for entry in entries.flatten() {
                ...
                && let Ok(modified) = metadata.modified()
            {
                ...
                {
                    eprintln!("gcode logger cleanup failed: {err}");
                }
            }
        }
    }
